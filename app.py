...
with st.sidebar:
    st.title("QuickCheck Health Diagnostics-QHD")
    
    diseases = ["Diabetes Detection","Heart Disease Detection","Malaria Detection", "Pneumonia Detection"]

    selected_diseases = st.selectbox("Choose a Disease to Diagnose", diseases)

  
if selected_diseases == "Diabetes Detection":
    
    st.title("DIABETES DISEASE DETECTOR")
    ...
    if st.button("RESULT"):
        prediction = diabetes_prediction(data=[glucose,blood_pressure,skin_thickness,insulin,bmi,pregnancies,dpf,age])
        
        if prediction==1:
            st.error("The patient has diabetes.Consult a Doctor!!")
        else:
...
if selected_diseases == "Heart Disease Detection":
    ...
    if st.button("RESULT"):
        ...
        if prediction==1:
            st.error(" Heart Disease Detected.Consult a Doctor!!")
        else:
...
if selected_diseases == "Malaria Detection":
    ...
    def preprocess_image(image_file):
        ...
        img = img.convert('RGB')  # Ensure the image has RGB format
    ...
    if st.button("RESULT"):
        ...
        if prediction[0][0] > 0.5:
            st.success("The Uploaded Image does not contain malaria parasites.")
        else:
            st.error("Malaria Parasites is Present.Consult a Doctor!!")
...
if selected_diseases == "Pneumonia Detection":
    ...
    if uploaded_file is not None:
        ...
        if st.button("RESULT"):
            ...
            # Display prediction
            if prediction[0][0] > 0.5:
                st.error("PNEUMONIA IS DETECTED.Consult a doctor!!")
            else:
                st.success("PNEUMONIA IS NOT AFFECTED.")
